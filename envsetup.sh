...
- cmremote: Add git remote for TS Gerrit Review.
- cmgerrit: A Git wrapper that fetches/pushes patch from/to TS Gerrit Review.
- cmrebase: Rebase a Gerrit change and push it again.
- aospremote: Add git remote for matching AOSP repository.
- cafremote: Add git remote for matching CodeAurora repository.
- mka:      Builds using SCHED_BATCH on all processors.
- mkap:     Builds the module(s) using mka and pushes them to the device.
- tska:     Cleans and builds using mka.
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ts_") ; then
       TS_BUILD=$(echo -n $1 | sed -e 's/^ts_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $TS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       TS_BUILD=
    fi
    export CM_BUILD
...
{
    target=$1
    TS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/shift/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the TS github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var TS_VERSION)
        ZIPFILE=ts-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.ts.device=$TS_BUILD");
    then
...
function installboot()
{
    ...
    then
        ...
        echo "The connected device does not appear to be $TS_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/shift/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
